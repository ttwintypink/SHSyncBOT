...
class RoleSync(commands.Cog):
    """
    Private -> Public role mirror.

    If user has PRIVATE SH:
      - add PUBLIC SH
      - remove PUBLIC FUN SH
    Else (including left/kicked/banned from private):
      - remove PUBLIC SH
      - add PUBLIC FUN SH
    """
    ...
    async def _apply_public_roles(self, member: discord.Member, want_sh: bool, reason: str) -> bool:
        ...
        try:
            ...
            log.error("Forbidden while editing roles for %s (%s). Check role hierarchy.", member, member.id)
            return False
        except discord.HTTPException:
    ...
    @app_commands.command(name="sync", description="Синхронизировать роли пользователя в паблике по роли SH в приватке")
    ...
    @app_commands.command(name="syncall", description="Сверить всех кандидатов (у кого SH/FUN SH в паблике)")
